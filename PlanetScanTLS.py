...
import PSTLSconstants as tls_constants
...
from PSTLSgrid import duration_grid, period_grid
...
from PSTLSvalidate import validate_inputs, validate_args
...
class transitleastsquares(object):
    ...
    def power(self, **kwargs):
        ...
        durations = numpy.arange(start=self.duration*(1-self.duration_range)*2/(self.period_max+self.period_min), stop=self.duration*(1+self.duration_range)*2/(self.period_max+self.period_min), step=self.duration*self.duration_grid_step*2/(self.period_max+self.period_min))
        #durations = duration_grid(
        #    periods, shortest=1 / len(self.t), log_step=self.duration_grid_step
        # )
        # print(str(durations*(self.period_max+self.period_min)/2))
